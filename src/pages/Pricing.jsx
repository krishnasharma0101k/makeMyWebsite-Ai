 ...
 const plans = [
  {
    ...
    price: "₹0",
    credits: 100,
    description: "Perfect for exploring makeMyWebsite-ai.",
    features: [
      "1 AI website generation/day",
    ...
    const handleBuy = async(planKey) =>{
      ...
      try {
        const result = await axios.post(`${serverUrl}/api/billing`,
          {plansType: planKey}, {withCredentials: true})
...
>
                {plans.map((p,i) => (
                    ...
                    className= {`relative rounded-3xl p-8 border backdrop-blur-xl transition-all
                        ${p.popular
                            ? "border-indigo-500 bg-gradient-to-b from-indigo-500/20 to-transparent shadow-2xl shadow-2xl shadow-indigo-500/30"
                            : "border-white/10 bg-white/5 hover:border-indigo-400 hover:bg-white/10"
                        }`}
                    
                    >
                      {p.popular && (
                        <span className='absolute top-5 right-5 px-3 py-1 text-xs rounded-full bg-indigo-500'>Popular</span>
                      )}

                      <h1 className='text-xl font-semibold mb-6'>{p.name}</h1>
